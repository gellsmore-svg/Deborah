...
# Constructs are UPPERCASE notation (SPEC/GRAMMAR). Case-insensitive matching
# against free prose re-types ordinary English ("Role clarity…", "Call the…")
# as constructs and strips the first word (review H2 / F1). Require uppercase.
_CONSTRUCT_LINE = re.compile(
    ...
    r"INSTITUTIONALIZE|SYMBOLIC_INTERACTION|CONFLICT|ACCOMMODATE|ASSIMILATE|ROLE|FEEDBACK|"
    r"MACRO|SAMPLE|VIEW)\b(.*)$",
)
_CONSTRUCT_STEP = re.compile(
    r"^(STEP|MILESTONE|ITERATE|RECURSE|QUEUE|PARALLEL|SERVICE|DECISION|RETRY|ERROR|AWAIT|CALL|"
    r"MERGE|REGULATION|APPRAISAL|DUAL_PROCESS|METACOGNITION|ALIGN|COALITION|RESISTANCE|"
    r"REINFORCEMENT|CASCADE|VISION|SOCIALIZE|INSTITUTIONALIZE|SYMBOLIC_INTERACTION|CONFLICT|"
    r"ACCOMMODATE|ASSIMILATE|ROLE|FEEDBACK|MACRO|SAMPLE|VIEW)\b(.*)$",
)
